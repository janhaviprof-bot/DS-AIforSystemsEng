# 03_agents_with_function_calling_activity.py
# Agents with Function Calling (activity)
# Pairs with 03_agents_with_function_calling.R
# Tim Fraser

# Same flow as the lecture script: tools include add_two_numbers, get_table, and average_array.
# Run from this folder: python 03_agents_with_function_calling_activity.py
# Tool implementations used by agent() also live in functions.py (see average_array there).
...
# Define a function to average a list of numbers (used as a tool)
...
# Define the tool metadata for average_array
tool_average_array = {
    "type": "function",
    "function": {
        "name": "average_array",
...
# 6. EXAMPLE 4: TOOL CALL #3 (AVERAGE) ###################################

messages = [
    {"role": "user", "content": "What is the average of 4, 8, and 12?"}
]

resp_avg = agent(messages=messages, model=MODEL, output="tools", tools=[tool_average_array])
print("🔧 Tool Call #3 (average) Result:")
print(resp_avg)
print()

if isinstance(resp_avg, list) and len(resp_avg) > 0:
    print(f"Average output: {resp_avg[0].get('output', 'No output')}")
    print()
